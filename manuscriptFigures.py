...
def figure2(simulationCollection):
    
    # create figure object
    fig = Figure("/home/aholaj/Nextcloud/figures_updated","figure2", ncols = 2, nrows = 3)
    ...
    for k in simulationList.getUCLALESSALSASimulations():
        simulationCollection[k].setLineWidth(matplotlib.rcParams["lines.linewidth"]*2)
        simulationCollection[k].setColor(Colorful.getDistinctColorList("green"))
    ...
    ax = fig.getAxes(1)
    ax.axis("off")
    legend_elements = [Patch(facecolor=simulationCollection["ICE0_8h"].getColor(),
                         label='UCLALES-SALSA'),
                   Patch(facecolor=simulationCollection["COSMO_ice0"].getColor(),
                         label='BIN'),
                   Patch(facecolor=simulationCollection["SAM-bin_ice0"].getColor(),
                         label='BULK')]
...
def figure3(simulationCollection):
    
    # create figure object
    fig = Figure("/home/aholaj/Nextcloud/figures_updated","figure3", ncols = 2, nrows = 1, figsize = [12/2.54, 7/2.54],bottom = 0.18, top=0.8)
    ...
    for k in simulationList:
        legend_elements.append(Patch(facecolor=simulationCollection[k].getColor(),
                         label=simulationCollection[k].getLabel()))
...
def figure4(simulationCollection):
    
    fig = Figure("/home/aholaj/Nextcloud/figures_updated","figure4", ncols = 2, nrows = 3, left=0.15, wspace=0.4)
    ...
    for k in simulationList:
        simulationCollection[k].getTSDataset()
        simulationCollection[k].getNCDataset()
        simulationCollection[k].setTimeCoordToHours()
    ...
    ax = fig.getAxes(2)
    for k in simulationList:
        simulationCollection[k].getNCDataset()["Ni_ii_vol"].plot(ax = ax,
                                                                 color = simulationCollection[k].getColor(),
                                                                 label =  simulationCollection[k].getLabel(),
                                                                 linewidth = simulationCollection[k].getLineWidth())
    end = 5
    ...
    for k in simulationList:
        for muuttuja in ["zb", "zc"]:
            simulationCollection[k].getTSDataset()[muuttuja].where(simulationCollection[k].getTSDataset()[muuttuja] > 0).plot(ax = ax,
                                                                 color = simulationCollection[k].getColor(),
                                                                 label =  simulationCollection[k].getLabel(),
                                                                 linewidth = simulationCollection[k].getLineWidth())
    end = 1000
    ...
    ax = fig.getAxes(5)
    ax.axis("off")
    legend_elements = [Patch(facecolor=simulationCollection["Prognostic_48h"].getColor(),
                         label='Prognostic ice'),
                   Patch(facecolor=simulationCollection["ICE4_24h"].getColor(),
                         label='ICE4')]
...
def figure5(simulationCollection):
    
    fig = Figure("/home/aholaj/Nextcloud/figures_updated","figure5", ncols = 2, nrows = 2, wspace=0.1, bottom = 0.14, left=0.15, top=0.98)
    
    simulation = "Prognostic_48h"
    
    simulationCollection[simulation].getPSDataset()
    simulationCollection[simulation].getTSDataset()
    simulationCollection[simulation].setTimeCoordToHours()
    ...
    for ind, muuttuja in enumerate(["P_cDUa", "P_cDUc", "P_cDUi"]):
        ax = fig.getAxes(ind)
        data = simulationCollection[simulation].getPSDataset()[muuttuja]
        data.values  = numpy.log10(data.values)
        
        im = data.plot.contourf("time","zt", ax = ax, levels=logaritmicLevels, add_colorbar = False)
        simulationCollection[simulation].getTSDataset()["zb"].plot(ax = ax,
                                                                 color = orange,
                                                                 linewidth = simulationCollection[simulation].getLineWidth())
        
        ax = Plot.getContourLine(ax, simulationCollection[simulation], "P_RHi", 100)
...
def figure7(simulationCollection):
    
    fig = Figure("/home/aholaj/Nextcloud/figures_updated","figure7", ncols = 2, nrows = 2, hspace=0.43, bottom = 0.14, left=0.15, top=0.98, wspace = 0.1)
...
def main(simulationDataFrameCSVFile):
    ...
    if False:
        figure2( simulationCollection )
    if False:
        figure3( simulationCollection )
    
    if False:
        figure4( simulationCollection )
        
    if False:
        figure5( simulationCollection )        
    if True:
        figure7( simulationCollection )        
    
    
    
    
       
def old(simulationDataFrameCSVFile):
...
if __name__ == "__main__":
    ...
    simulationDataFrameCSVFile = "/home/aholaj/Nextcloud/figures_updated/manuscriptSimulationData.csv"
